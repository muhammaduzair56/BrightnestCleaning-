...
const articles: Article[] = [
  ...
  },
  {
    ...
    image: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/WSrzKHVuIOAmKyVt.jpg",
    socialImage: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/WSrzKHVuIOAmKyVt.jpg",
  ...
  },
  {
    ...
    image: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/fsUPkazEBAbqgGWT.jpg",
    socialImage: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/fsUPkazEBAbqgGWT.jpg",
  ...
  },
  {
    ...
    image: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/SgTYAWsiaYNJbPQn.jpg",
    socialImage: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/SgTYAWsiaYNJbPQn.jpg",
  ...
  },
  {
    ...
    image: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/ahXOeRXjbQLAKDbu.jpg",
    socialImage: "https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/ahXOeRXjbQLAKDbu.jpg",
...
function ShareArticle({ article }: { article: Article }) {
  ...
  return <section className="mt-14 border-t border-[#173137]/12 pt-8 sm:mt-20"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#2f9f91]">Pass it on</p><h2 className="font-display mt-2 text-[30px] leading-none tracking-[-0.04em]">Share this note</h2></div><div className="flex flex-wrap gap-2" aria-label="Share this article"><a href={`https://wa.me/?text=${encodedTitle}%20${encodedUrl}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#173137]/15 px-4 py-2.5 text-xs font-extrabold text-[#173137] transition-colors hover:border-[#2f9f91] hover:text-[#2f9f91]">WhatsApp</a><a href={`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#173137]/15 px-4 py-2.5 text-xs font-extrabold text-[#173137] transition-colors hover:border-[#2f9f91] hover:text-[#2f9f91]">Facebook</a><a href={`https://twitter.com/intent/tweet?text=${encodedTitle}&url=${encodedUrl}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#173137]/15 px-4 py-2.5 text-xs font-extrabold text-[#173137] transition-colors hover:border-[#2f9f91] hover:text-[#2f9f91]">X</a><button type="button" onClick={copyLink} className="inline-flex items-center gap-2 rounded-full bg-[#173137] px-4 py-2.5 text-xs font-extrabold text-[#f8f6ef] transition-colors hover:bg-[#2f9f91]" aria-label="Copy article link">{copied ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}{copied ? "Copied" : "Copy link"}</button></div></div></section>;
}
    ...
    <header className="sticky top-0 z-40 px-3 pt-3 sm:px-5 lg:px-8">
      <div className="mx-auto flex min-h-[72px] max-w-[1440px] items-center justify-between gap-5 rounded-full border border-[#173137]/12 bg-[#fffdf7]/95 px-4 shadow-[0_10px_30px_rgba(23,49,55,0.08)] backdrop-blur-xl sm:min-h-[76px] sm:px-5 lg:px-7">
        <Link href="/" className="group flex shrink-0 items-center" aria-label="BrightNest Cleaning UK home">
          <img src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/nGxiaCTVOHaPBQzw.png" alt="BrightNest Cleaning UK logo" className="h-[62px] w-[166px] origin-left scale-[1.12] object-contain object-left transition-transform duration-200 group-hover:scale-[1.16] group-active:scale-95 sm:h-[68px] sm:w-[184px]" />
        </Link>
        ...
        <div className="hidden items-center gap-3 lg:flex"><Link href="/dashboard" className="text-sm font-bold text-[#173137]/70 transition-colors hover:text-[#2f9f91]">My bookings</Link><Link href="/#booking" className="btn-primary">Book a clean <ArrowRight className="h-4 w-4" /></Link></div>
      ...
      {mobileOpen && <div id="blog-mobile-navigation" className="border-t border-[#173137]/10 bg-[#f8f6ef] px-5 py-4 shadow-xl lg:hidden"><nav className="flex flex-col" aria-label="Mobile navigation">{navigation.map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`flex items-center justify-between border-b border-[#173137]/10 py-4 text-left text-base font-bold ${label === "Blog" ? "text-[#2f9f91]" : ""}`} aria-current={label === "Blog" ? "page" : undefined}>{label}<ArrowRight className="h-4 w-4 text-[#2f9f91]" /></Link>)}<Link href="/dashboard" onClick={() => setMobileOpen(false)} className="flex items-center justify-between border-b border-[#173137]/10 py-4 text-left text-base font-bold">My bookings <ArrowRight className="h-4 w-4 text-[#2f9f91]" /></Link><Link href="/#booking" onClick={() => setMobileOpen(false)} className="btn-primary mt-4 w-full justify-center">Book a clean <ArrowRight className="h-4 w-4" /></Link></nav></div>}
    </header>
  </>;
}

function Footer() {
  return <footer className="bg-[#173137] px-5 pb-7 pt-10 text-[#f8f6ef] sm:pt-14 lg:px-10 lg:pt-18"><div className="mx-auto grid max-w-[1440px] gap-8 border-b border-white/15 pb-9 sm:gap-10 sm:pb-12 md:grid-cols-[1.25fr_0.75fr_0.75fr]"><div><img src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663898260788/nGxiaCTVOHaPBQzw.png" alt="BrightNest Cleaning UK logo" className="h-[100px] w-[218px] origin-left scale-[1.08] object-contain object-left" /><p className="mt-6 max-w-[360px] text-sm leading-7 text-white/60">Thoughtful domestic and specialist cleaning across Birmingham and surrounding areas.</p></div><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#9ee0d2]">Explore</p><div className="mt-5 flex flex-col gap-3 text-sm font-bold text-white/70"><Link href="/">Home</Link><a href="/#services">Services</a><a href="/#how-it-works">How it works</a><Link href="/blog" className="text-[#9ee0d2]">Blog</Link><a href="/#faqs">FAQs</a></div></div><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#9ee0d2]">Book</p><p className="mt-5 text-sm leading-7 text-white/60">Need a tailored home or specialist cleaning request?</p><Link href="/#booking" className="mt-4 inline-flex items-center gap-2 text-sm font-extrabold text-white underline decoration-[#9ee0d2] decoration-2 underline-offset-4">Start your request <ArrowRight className="h-4 w-4" /></Link></div></div><div className="mx-auto flex max-w-[1440px] flex-col gap-3 pt-6 text-[11px] font-bold text-white/40 sm:flex-row sm:items-center sm:justify-between"><span>© {new Date().getFullYear()} BrightNest Cleaning UK. All rights reserved.</span><div className="flex flex-wrap items-center gap-x-3 gap-y-1"><Link href="/privacy-policy">Privacy Policy</Link><span aria-hidden="true">·</span><Link href="/terms-of-service">Terms of Service</Link><span aria-hidden="true">·</span><Link href="/dashboard">My bookings</Link></div></div></footer>;
}

function BlogIndex() {
  return <main><section className="px-5 pb-12 pt-6 sm:pb-18 sm:pt-8 lg:px-10 lg:pb-24 lg:pt-10"><div className="mx-auto grid max-w-[1440px] overflow-hidden rounded-[28px] border border-[#173137]/10 bg-[#e7f0e9] lg:grid-cols-[0.83fr_1.17fr] lg:rounded-[42px]"><div className="flex min-h-[420px] flex-col justify-between p-7 sm:p-10 lg:min-h-[530px] lg:p-14 xl:p-16"><div className="flex items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#2f9f91] sm:text-xs"><span className="nestline" aria-hidden="true" /><span>BrightNest notes</span></div><div className="max-w-[570px]"><p className="eyebrow">The home journal</p><h1 className="font-display mt-5 text-[52px] leading-[0.92] tracking-[-0.065em] sm:text-[68px] xl:text-[82px]">Cleaning advice for a lighter week.</h1><p className="mt-6 max-w-[470px] text-base leading-7 text-[#173137]/70 sm:text-lg">Practical notes for homes, hosts and busy households across Birmingham — written to make the next clean feel easier to plan.</p><Link href="/#booking" className="btn-primary mt-8">Plan your clean <ArrowRight className="h-4 w-4" /></Link></div></div><div className="relative min-h-[300px] overflow-hidden bg-[#c5dfd5] lg:min-h-full"><img src="/blog/brightnest-blog-winter-home.webp" alt="A calm, clean kitchen corner with natural daylight" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-r from-[#173137]/15 via-transparent to-transparent" /><div className="absolute bottom-5 left-5 max-w-[250px] rounded-[18px] border border-white/40 bg-[#f8f6ef]/88 p-4 backdrop-blur-sm sm:bottom-8 sm:left-8"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#2f9f91]">A considered approach</p><p className="mt-2 text-sm font-bold leading-5 text-[#173137]">Useful detail, without the overwhelm.</p></div></div></div></section><section className="px-5 pb-16 sm:pb-24 lg:px-10 lg:pb-32"><div className="mx-auto max-w-[1440px]"><div className="flex flex-col gap-5 border-t border-[#173137]/15 pt-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">From the journal</p><h2 className="font-display mt-4 max-w-[620px] text-[42px] leading-[0.98] tracking-[-0.055em] sm:text-[56px]">Small notes. Noticeable difference.</h2></div><p className="max-w-[340px] text-sm leading-6 text-[#173137]/65 sm:text-right">Eight concise reads for the everyday rhythm of keeping a home, guest space or workplace feeling ready.</p></div><div className="mt-10 grid gap-x-6 gap-y-12 sm:grid-cols-2 lg:mt-14 lg:grid-cols-4">{articles.map((article) => <article key={article.id} className="group"><Link href={`/blog/${article.id}`} className="block rounded-[22px] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#2f9f91]/25"><div className="relative aspect-[1.12] overflow-hidden rounded-[22px] bg-[#e7f0e9]"><img src={article.image} alt="" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.035]" /><div className="absolute left-4 top-4 rounded-full bg-[#f8f6ef]/90 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#173137]">{article.category}</div></div><div className="mt-5 flex items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#2f9f91]"><span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{article.date}</span><span aria-hidden="true" className="text-[#173137]/25">·</span><span className="inline-flex items-center gap-1 text-[#173137]/50"><Clock3 className="h-3.5 w-3.5" />{article.readTime}</span></div><h3 className="mt-3 text-xl font-extrabold leading-[1.08] tracking-[-0.035em] text-[#173137]">{article.title}</h3><p className="mt-3 text-sm leading-6 text-[#173137]/65">{article.excerpt}</p><span className="mt-4 inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.12em] text-[#2f9f91]">Read article <ArrowRight className="h-3.5 w-3.5 transition-transform duration-200 group-hover:translate-x-0.5" /></span></Link></article>)}</div></div></section><section className="px-5 pb-12 sm:pb-16 lg:px-10 lg:pb-20"><div className="mx-auto grid max-w-[1440px] gap-6 rounded-[26px] bg-[#173137] p-7 text-[#f8f6ef] sm:p-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:rounded-[38px] lg:p-14"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#9ee0d2]">Need a hand with the detail?</p><h2 className="font-display mt-4 max-w-[650px] text-[42px] leading-[0.98] tracking-[-0.055em] sm:text-[56px]">Turn a good intention into a calmer home.</h2></div><div className="lg:justify-self-end"><p className="max-w-[360px] text-sm leading-6 text-white/62">Tell us what needs attention, where you are based and the timing that suits you. We’ll review the details before confirming a visit.</p><Link href="/#booking" className="mt-6 inline-flex items-center gap-2 text-sm font-extrabold text-white underline decoration-[#9ee0d2] decoration-2 underline-offset-4">Start your request <ArrowRight className="h-4 w-4" /></Link></div></div></section></main>;
}

function ArticleView({ article }: { article: Article }) {
  return <main><section className="px-5 pb-10 pt-6 sm:pb-16 sm:pt-8 lg:px-10 lg:pb-20 lg:pt-10"><div className="mx-auto grid max-w-[1180px] overflow-hidden rounded-[28px] border border-[#173137]/10 bg-white lg:grid-cols-[0.9fr_1.1fr] lg:rounded-[38px]"><div className="flex flex-col justify-between p-7 sm:p-10 lg:p-14"><Link href="/blog" className="inline-flex w-fit items-center gap-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[#2f9f91]"><ArrowLeft className="h-4 w-4" />Back to journal</Link><div className="mt-16"><p className="eyebrow">{article.category}</p><h1 className="font-display mt-5 text-[48px] leading-[0.94] tracking-[-0.06em] sm:text-[64px]">{article.title}</h1><div className="mt-7 flex flex-wrap items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#2f9f91]"><span className="inline-flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{article.date}</span><span className="text-[#173137]/25">·</span><span className="inline-flex items-center gap-1 text-[#173137]/50"><Clock3 className="h-3.5 w-3.5" />{article.readTime}</span></div></div></div><div className="relative min-h-[360px] bg-[#e7f0e9] lg:min-h-full"><img src={article.image} alt="" className="absolute inset-0 h-full w-full object-cover" /></div></div></section><article className="px-5 pb-16 sm:pb-24 lg:px-10 lg:pb-32"><div className="mx-auto max-w-[820px]"><p className="font-display text-[30px] leading-[1.12] tracking-[-0.04em] text-[#173137] sm:text-[38px]">{article.intro}</p><div className="mt-12 space-y-12 border-t border-[#173137]/12 pt-10 sm:mt-16 sm:space-y-14 sm:pt-12">{article.sections.map((section) => <section key={section.heading}><h2 className="font-display text-[32px] leading-none tracking-[-0.045em] sm:text-[42px]">{section.heading}</h2><div className="mt-5 space-y-4 text-base leading-8 text-[#173137]/70 sm:text-lg">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>{section.points && <ul className="mt-6 grid gap-3 rounded-[22px] bg-[#e7f0e9] p-5 text-sm font-bold leading-6 text-[#173137] sm:grid-cols-3 sm:p-6">{section.points.map((point) => <li key={point} className="flex gap-2"><Check className="mt-1 h-4 w-4 shrink-0 text-[#2f9f91]" />{point}</li>)}</ul>}</section>)}</div><div className="mt-14 rounded-[26px] bg-[#173137] p-7 text-[#f8f6ef] sm:mt-20 sm:p-10"><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[#9ee0d2]">Make the next clean feel easier</p><h2 className="font-display mt-4 max-w-[570px] text-[36px] leading-[0.98] tracking-[-0.05em] sm:text-[48px]">Tell us what your home needs.</h2><Link href="/#booking" className="btn-primary mt-7 bg-[#9ee0d2] text-[#173137] hover:bg-white">Plan your clean <ArrowRight className="h-4 w-4" /></Link></div></div><ShareArticle article={article} /></article></main>;
}
