...
import { ArrowLeft, ArrowUpRight, CalendarDays, CheckCircle2, Clock3, KeyRound, LogOut, Mail, RefreshCw, X } from "lucide-react";
...
function BookingCard({ booking, token, onChanged }: { booking: CustomerBooking; token?: string; onChanged?: () => void }) {
  ...
  return <article className="rounded-[24px] border border-[#173137]/10 bg-white p-5 shadow-[0_18px_50px_rgba(23,49,55,0.06)] sm:p-6">
    ...
    <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-t border-[#173137]/10 pt-4 text-sm font-bold text-[#173137]/65"><span className="inline-flex items-center gap-2"><Clock3 className="h-4 w-4 text-[#2f9f91]" /> {booking.preferred_time.slice(0, 5)}</span><span>{booking.frequency}</span></div>
...
function BookingGroup({ title, intro, bookings, empty, token, onChanged }: { title: string; intro: string; bookings: CustomerBooking[]; empty: string; token?: string; onChanged?: () => void }) {
  return <section><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end"><div><p className="eyebrow">{title}</p><p className="mt-2 text-sm text-[#173137]/60">{intro}</p></div><span className="text-sm font-extrabold text-[#2f9f91]">{bookings.length} {bookings.length === 1 ? "booking" : "bookings"}</span></div><div className="mt-4 grid gap-4">{bookings.length ? bookings.map((booking) => <BookingCard key={booking.id} booking={booking} token={token} onChanged={onChanged} />) : <div className="rounded-[24px] border border-dashed border-[#173137]/20 bg-[#edf3ed] p-6 text-sm font-bold leading-6 text-[#173137]/60">{empty}</div>}</div></section>;
}
