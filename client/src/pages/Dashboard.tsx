 ...
 */
import { CustomerBooking, CustomerDashboard, configuredApiUrl, customerApi } from "@/lib/api";
import { ArrowLeft, ArrowUpRight, CalendarDays, CheckCircle2, Clock3, KeyRound, LogOut, Mail } from "lucide-react";
...
function BookingCard({ booking }: { booking: CustomerBooking }) {
  return (
    <article className="rounded-[24px] border border-[#173137]/10 bg-white p-5 shadow-[0_18px_50px_rgba(23,49,55,0.06)] sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#2f9f91]">{formatDate(booking.preferred_date)}</p>
          <h3 className="font-display mt-2 text-[28px] leading-none tracking-[-0.055em] text-[#173137]">{booking.service_type}</h3>
        </div>
        <span className={`rounded-full px-3 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.1em] ${statusTones[booking.status]}`}>{statusLabels[booking.status]}</span>
      </div>
      <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-t border-[#173137]/10 pt-4 text-sm font-bold text-[#173137]/65">
        <span className="inline-flex items-center gap-2"><Clock3 className="h-4 w-4 text-[#2f9f91]" /> {booking.preferred_time.slice(0, 5)}</span>
...
function BookingGroup({ title, intro, bookings, empty }: { title: string; intro: string; bookings: CustomerBooking[]; empty: string }) {
  return (
    <section>
      <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
        <div><p className="eyebrow">{title}</p><p className="mt-2 text-sm text-[#173137]/60">{intro}</p></div>
        <span className="text-sm font-extrabold text-[#2f9f91]">{bookings.length} {bookings.length === 1 ? "booking" : "bookings"}</span>
      </div>
      <div className="mt-4 grid gap-4">{bookings.length ? bookings.map((booking) => <BookingCard key={booking.id} booking={booking} />) : <div className="rounded-[24px] border border-dashed border-[#173137]/20 bg-[#edf3ed] p-6 text-sm font-bold leading-6 text-[#173137]/60">{empty}</div>}</div>
    </section>
  );
}
...
export default function Dashboard() {
  ...
  const loadDashboard = async (activeToken: string) => {
    ...
    setError("");
    try {
      setDashboard(await customerApi.bookings(activeToken));
    } catch (requestError) {
      sessionStorage.removeItem("brightnest_customer_access");
      setToken("");
      setDashboard(null);
      setError(requestError instanceof Error ? requestError.message : "Your dashboard could not be loaded.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (oneTimeToken) {
      void customerApi.exchange(oneTimeToken).then((response) => {
        sessionStorage.setItem("brightnest_customer_access", response.access_token);
        window.history.replaceState({}, document.title, "/dashboard");
        setToken(response.access_token);
        return loadDashboard(response.access_token);
      }).catch((requestError) => {
        setLoading(false);
        setError(requestError instanceof Error ? requestError.message : "This dashboard link is invalid or expired.");
        window.history.replaceState({}, document.title, "/dashboard");
      });
    } else if (token) {
  ...
  const requestLink = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLinkSent(false);
    try {
      await customerApi.requestAccess(email.trim());
      setLinkSent(true);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "We could not request a dashboard link.");
    } finally {
      setLoading(false);
    }
  };
