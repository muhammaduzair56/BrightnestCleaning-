...
export default function Dashboard() {
  ...
  const [error, setError] = useState("");
  const [dataAction, setDataAction] = useState<"export" | "delete" | "">("");
  ...
  const requestDataAction = async (requestType: "export" | "delete") => { if (!token) return; if (requestType === "delete" && !window.confirm("Please confirm that you want BrightNest to review a request to delete your customer data.")) return; setDataAction(requestType); setDataMessage(""); try { if (requestType === "export") { const exported = await customerApi.exportData(token); const blob = new Blob([JSON.stringify(exported, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "brightnest-data-export.json"; document.body.appendChild(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(url); setDataMessage("Your data export has been downloaded."); } else { const response = await customerApi.requestData(token, "delete"); setDataMessage(response.message); } } catch (requestError) { setDataMessage(requestError instanceof Error ? requestError.message : "We could not complete that request."); } finally { setDataAction(""); } };
